...
open(f"/tmp/tg-{kv}-{label}.txt","w").write(c)
...
for kv in "$@"; do
  ...
  a=$(cat /tmp/tg-$kv-plain.txt 2>/dev/null); b=$(cat /tmp/tg-$kv-mtp3.txt 2>/dev/null); c=$(cat /tmp/tg-$kv-mtp7.txt 2>/dev/null)
