...
> `0016..0030`, still skipping `0015`).  `0027` = the meta-backend `graph_optimize` forwarding;
...
> fresh r13+beta worktree + all 29 in order (`for p in .../patches/0*.patch; do` skip `0015`, `git am
> "$p"`; `done`) applies **29/29** and reproduces tree
