...
| [`gfx1201-closing.md`](gfx1201-closing.md) | **the live gfx1201 (3× R9700) brief: the remaining open items** (OP-1 the MTP CPU-spin automatic fix, OP-2 the throughput re-baseline, OP-3 the `0028` follow-ups, …) + the operational apply/build/gate instructions for a fresh session. |
...
> **Current set (2026-09-24): the campaign is now 27 patches** (`0001..0014` + `0016..0028`, still
> skipping `0015`).  `0027` = the meta-backend `graph_optimize` forwarding; `0028` = the MMVQ↔MMQ band
> boundary (the qwen4exp `n_max` 7→8 fix), see [`gfx1151-closing.md`](gfx1151-closing.md).  A fresh
> r13+beta worktree + all 27 in order (`for p in .../patches/0*.patch; do` skip `0015`, `git am "$p"`;
> `done`) applies **27/27** and reproduces tree `533eee3188ab7df9b6cf394adeaa31b46bd13ff2`.
