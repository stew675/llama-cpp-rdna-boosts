# MTP qualification — ours (adaptive) vs pwilkin's (fixed), qwen4exp IQ4_NL (2026-09-21)

**Status:** preliminary, one session, single box. Enough to answer "is our MTP behind his?", not a
tuned four-axis table. Runs the 2026-09-21 `wip/closing-the-gap` investigation.
...
| pwilkin | `~/pwilkin-llama-cpp` @ `b0f31f587`, **rebuilt** 2026-09-21 (the `git pull` had not been rebuilt; the old binary was `f5daaa3cf`) |
...
pwilkin's IQ4_NL directory ships `mtp-Qwen3.8-Flash-Next-shared-Q8_0.gguf`, which declares
...
controller is inert. Pwilkin's build runs the same head clean (`0.970, 0.928, 0.878`), so this is **our
...
| axis | ours **plain** | his **plain** | ours `draft-mtp n3` | ours `adaptive 9` | ours `adaptive 12` | his `draft-mtp` |
|---|---:|---:|---:|---:|---:|---:|
...
| axis | ours `n3` | ours `adaptive 9` | ours `adaptive 12` | his fixed |
|---|---:|---:|---:|---:|
...
1. **Our plain (non-MTP) decode is ahead of his** on all three axes (+4 % code, +6 % prose, +2 % recall).
   Any absolute MTP comparison flatters his stack by that amount; the speedup table is the fair one.
2. **At fixed depth the MTP speedup is at parity** (ours `n3` 1.90/1.78/2.05 vs his 1.91/1.79/2.02).
   His tree has no adaptive controller (`common/speculative-adaptive.h` is absent; fixed `n_max` plus
   upstream's within-round `p_min`/`n_min` stop only), so he did **not** adapt ours, and he is not ahead.
...
4. **The only MTP-side gap is correctness/compat: `nextn_shared_target_tensors` support** (above). Until
   it lands, the MTP head pwilkin's own model ships cannot be used on our delivery at all.
