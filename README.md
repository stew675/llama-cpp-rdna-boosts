...
- **Block-14 amendment — QSA quantized-KV decode gate + derived-cache
  pool gate (2026-09-07):** a
  ...
  falls back to the per-op chain for quantized indexer keys, and the
  derived-cache F32 block-vector pool (~103 MiB at a 70k ctx) is now
  allocated only when the fused path can actually use it (float keys +
  `GGML_CUDA_QSA_INDEXER_CACHE` set) instead of for every context.
  Validated
  ...
  path unregressed; pool-absent vs derived-engaged decode byte-identical).
  Fork tip moved `bfcc4be99`; set regenerated.
