...
|---|---|---|---|
| **V3** | derived kq mask for the plain attention path: stop materialising the `n_kv × n_tps` F16 mask and its host mirror; derive visibility in the FA prefill/MMA kernel from compact per-cell state (packed mask kept for decode, small batches and every unsupported case).  **Phases 1+2a+2b DONE 2026-09-10**: the predicate is proven bit-exact on the host (incl. the SWA and non-causal paths), the op + a CPU reference exist, and the CUDA MMA derived path is implemented and validated on its own (6/6 derived `test-backend-ops` cases on CPU, 3/3 on ROCm0, 5104/5104 on the whole FA suite).  Remaining: phase 2c (graph plumbing + probe + end-to-end validation) | **−800 MiB/GPU VRAM − 800 MiB host** (27B 1920.33 → ~1120 compute, 4B 1800.33 → ~1000) | **`wip/arch-independent-memory/V3-DERIVED-KQ-MASK-PLAN.md`** (verified predicate + phase-2 spec + §4 progress); `DERIVED-MASK-DESIGN.md` §2–§5 + §7; `HANDOVER.md` §3.1 |
