#!/usr/bin/env bash
# Apply the full rdna-boosts patch set (blocks 01-15)
...
APPLIED_WITH_3WAY=0
if ! git am "$PATCHES"/000[1-9]-*.patch "$PATCHES"/001[0-5]-*.patch; then
    echo "strict 'git am' failed at this base; aborting and retrying the series with 'git am -3' (3-way merge)" >&2
    git am --abort >/dev/null 2>&1 || true
    git am -3 "$PATCHES"/000[1-9]-*.patch "$PATCHES"/001[0-5]-*.patch
    APPLIED_WITH_3WAY=1
fi
