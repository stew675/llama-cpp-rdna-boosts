...
# HIP all-reduce + RDNA4 gate) is the fork's working-tree delta vs
# blocks-tip over the four allreduce files.  The fork's working tree must
# carry the block-12 changes (the WIP allreduce-hip.cu/allreduce.cu/
# allreduce.cuh/ggml-cuda.cu modifications).
set -euo pipefail
