...
arm the path to the RDNA4 GQA-6 decode/verify band, removing the original opt-in reason ("no
...
native arm, bf16) now takes the RDNA4 GQA-6 decode/verify band too.  The band gate is no longer
"has a native read": f16 has none, but at verify widths the F1 purity pin
(`cols_per_block = ncols2 > 2 ? ncols2 : 2`) makes the whole `n_q <= 8` band use the `n_q = 1` tile
config, and with GQA 6 (`ncols2 = 2`) every query row re-reads and re-stages every K/V element three
times (gfx1201 kv 102400 f16: `n_q = 1` ~604 GB/s, `n_q = 4` ~180 GB/s).  The band's config is now
chosen per K/V element size: native-quantized keeps `ncols1 = 4` / `P = nsm`, the 2-byte types take
`ncols1 = 2` / `P = max(2, 3*nsm/4)` (the dequantization-free work hides fewer unused columns and
...
band.  At head 256 with GQA 6 the tile kernel can only fold `ncols2 = 2` (its `ncols2` must divide
the GQA ratio), so the whole `n_q <= 8` band fetched and dequantized every K/V element once per head
pair — three times per query row.  The band now routes the whole band (`n_q = 1` included) to the
existing WMMA `(256, ncols1 4|2, ncols2 8)` instances and splits the KV round-robin over a fixed
`P = nsm` blocks per output tile, which depends on neither `n_q` nor the KV length and so preserves
the greedy-purity band invariant.  Coverage follows `ggml_cuda_fattn_kv_native_type`, so q8_0, q4_0,
q4_1, q5_0, q5_1 and iq4_nl all win (f16/bf16 stay on tile).  Default ON; `GGML_HIP_FA_BAND_WMMA=0`
opts out.  Op level kv 16384 q8_0: `n_q` 3 326 -> 150, `n_q` 8 814 -> 230 µs; end-to-end 27B
`draft-mtp` n3 at ~40k +13-18 % across 1/2/3 GPUs; prefill flat.  See `WORKLOG.md` (2026-09-25 r4).
**r3
...
under tensor split and users had to size `-c`/`-ngl`/`-ts` by hand.  The Meta device's accessors are now
...
(2026-09-20, issue #42) makes the compute reserve account for the reachable (packed) kq mask**
(only block 15 changed): V3's derived form is a per-*batch* decision, so a batch it cannot serve — 2-D
M-RoPE (image/audio chunks) or a multi-sequence batch — allocates the packed mask at runtime, which the
reserve (measured with the derived form on) did not contain.  At depth that mask is hundreds of MiB, so
the first such batch had to **grow** the compute buffer mid-run; under `--fit on --fit-target 256` the
growth fails (`cudaMalloc failed: out of memory`) and the request dies with `failed to process mtmd
chunk`, and a following request then tripped a `ggml_backend_tensor_alloc` bounds assert on the
allocator state the failed reserve left behind.  `sched_reserve()` now measures with the packed mask
when — and only when — such a batch is reachable in that context (the new
`kq_mask_packed_reachable()`: M-RoPE or `n_seq_max > 1`; every other packed-mask source already keeps
the mask in the reserve), so `--fit` counts it where it can happen and the growth cannot occur.  On the
reporter's M-RoPE model that costs **8960 tokens / -4.4 %** of fitted context; a non-M-RoPE
single-sequence model keeps V3's reserve unchanged (gemma4-E4B 113.94 MiB derived vs 146.80 forced
packed), and same-seed output is byte-identical with throughput unchanged everywhere.  An independent
`ggml-alloc`
hardening turns any remaining buffer-allocation failure into a clean `GGML_STATUS_ALLOC_FAILED`
instead of the assert.  See the 2026-09-20 block-15 (r11) amendment section below.  **r10
(2026-09-20, issue #41) fixes block 11's pre-fill test** (only block 11 changed):
`cgraph->nodes[0]->ne[1] > 1` misread a split-MoE one-token decode split's leading expert tensor
(`[n_ff, n_expert_used, 1]`, so `ne[1] == n_expert_used == 10`) as a multi-token batch, so with
`-ncmoe` decode never captured/warmed/replayed a HIP graph (`GGML_CUDA_DISABLE_GRAPHS=1` was a no-op).
A new `ggml_cuda_graph_is_multi_token()` reads the token count from `MUL_MAT_ID`'s `ne[2]` / a weight
`MUL_MAT`'s `src1->ne[1]`, with the old test kept as the fallback, and
`ggml_cuda_graph_update_executable()` now destroys/re-instantiates the exec on HIP instead of leaking
through `hipGraphExecUpdate` (ROCm <= 10.0, rocm-systems#10713; `GGML_HIP_GRAPH_FORCE_UPDATE=1`
opt-out).  On the reporter's Qwen3.8-Flash-Next UD-Q4_K_XL (gfx1201): **0 -> 50** graph warmups and
**0 -> 687** replays in a 16-token decode, `tg` **10.6 -> 12.8 t/s** (~+20 %), greedy output
byte-identical; dense and pre-fill unchanged.  **r9 (2026-09-19) implements block-15 V3's derived kq
...
tile kernel for heads > 256 (stock's choice); the head-256 WMMA row itself is a **+44-52 %**
deep-prefill win over tile on gfx1100, so it stays.  RDNA4 (576) and RDNA3_5 (320) are untouched.
See the 2026-09-18 block-04 (r5) amendment section below.  **r4 (2026-09-18) fixes the gfx1100 tensor-split deep-prefill regression
...
it and so carry new `From <sha>`/`index` lines with unchanged bodies, and blocks 00-05 are byte-identical
to r11.  The net `rdna-boosts-all.patch` is byte-identical to the r10/r11-era net diff, which is the
check that only the block *distribution* moved.  The campaign record moved from `../beta/tensor-fit-fix/`
to `../archive/work/tensor-fit-fix/` with this promotion.

**The gap.**  `common_params_fit_impl()` opened with
`throw common_params_fit_exception("llama_params_fit is not implemented for SPLIT_MODE_TENSOR, abort")`,
so `--fit` (default **on**) was a silent no-op under `-sm tensor`: `common_fit_params()` catches the
exception and the run continues with the user's `-c` / `-ngl` / `-ts` untouched.  That is documented
upstream behaviour since the fit-params PR #22171, but it means the flag a user expects to be protecting
them simply is not running.

**Why it was not a small change.**  Under `-sm tensor` with more than one device,
`llama_prepare_model_devices()` wraps every GPU in a single **Meta** device, so the existing fit
algorithm is unusable for it twice over: it is layer-granular (`tensor_split[id] = n_layer`, whole-tensor
CPU overflow), which cannot express "shard every layer across the devices"; and the Meta row of
`llama_get_memory_breakdown()` mixes semantics, with `model` counting the whole offloaded tensor set once
while `context` / `compute` are the **largest per-device sub-buffer**.

**The change** (4 files, +174 lines in `common/fit.cpp`).  Two ggml accessors are exposed
(`ggml_backend_dev_is_meta()`, `ggml_backend_meta_dev_n_devs()` / `ggml_backend_meta_dev_simple_dev()`;
they already existed upstream, file-static), and `common/fit.cpp` gains a dedicated tensor-split path:
expand the Meta device into its simple devices; `target_i = free_i - margin_i` from the per-device free
memory and `--fit-target`; set `tensor_split[i] ∝ target_i` unless the user pinned `-ts`, in which case
honour the requested balance as a constraint and log the resulting `effective budget`
(`min_i(target_i / r_i)`, which for the auto split reduces to `sum(target)`); estimate per-device use as
`model + n_devices * (context + compute)` (plus the draft/embedded MTP model, measured the same way and
zeroed when `shares_model`); then reduce an auto `n_ctx` by interpolation and, if that is still not
...
only overlap is `ggml/src/ggml-backend-meta.cpp`, which blocks 09/14/15 also touch, at lines ~790-2600
versus this change's ~83/~196; the replay of blocks 07-15 on the amended block 06 applies cleanly and the
resulting tree is byte-identical to the earlier block-15 placement.  The alternative home is `upstream/` as
an `UPSTREAM-PR-*` candidate, which remains worth doing: the change is generic llama.cpp.

**Re-validated on r11 before promotion (2026-09-21)**, because the patch was written against the r5/r6-era
tree and r11 changes what the fit must reserve (the reachable packed kq mask now sits in the reserve for
M-RoPE models).  Hardware 2x/3x R9700 (gfx1201, 32 GiB), ROCm 7.14.1.

* **Fit decisions.**  Unchanged where the budget is comfortable and **more conservative** where it binds.
  The default cases reproduce the 2026-09-18 record exactly: 27B Q8_0 2 GPU `-c 4096` -> `-c 4096 -ngl -1
  -ts 31254,31254` (identical), 3 GPU -> `-ts 31254,31254,31254` (identical), auto-ctx
  `--fit-target 16000,16000` -> estimate 59899 vs 32556 target, `n_ctx` 262144 -> **43264** (the beta:
  59901 -> 43264, i.e. identical).  The `-ngl`-reduction cases go lower than the beta (`--fit-target
  30000 x3`: 12 -> **4**; pinned `-ts 1,3 --fit-target 26000x2`: 17 -> **7**; MoE 35B-A3B
  `--fit-target 26000x2`: 14 -> **11**) because the fit must now size for the mask that r11 restores for
  M-RoPE; its internal trace is monotonic and lands under the budget in every case (`-ngl 4 -> 6644 <=
  6834`, `-ngl 7 -> 8328`, etc.), which is the conservative direction.
* **End-to-end loads (7 configs, all with `--fit on -sm tensor`).**  27B Q8_0 comfortable / auto-ctx /
  +embedded MTP / 3 GPU, 27B + **separate** MTP head with `--spec-type draft-mtp-adaptive` (the issue #38
  ...
  compute-buffer growth; MTP acceptance 1.0 (embedded) and 0.5 (adaptive + separate head).  The load-time
  memory breakdown shows the `compute` figure tracking context (126 MiB at `-c 4096` -> 378 MiB at
...
* **Coherence gate.**  With the fit deactivated (`--fit off`, explicit `-c 8192 -ngl 99 -ts 1,1`) and a
  fixed seed, r11 and r11+this-patch produce **byte-identical** output
...
* **Balance + single device.**  Asymmetric `--fit-target 1024,8000` (fit chooses `-ts 31254,24278`): peak
  per device by `rocm-smi` **15900 / 11994 MiB**, against the 2026-09-18 record's 15925 / 12018 (within
  0.2 %), and ~3 % above the ideal proportional split (the loader's per-tensor rounding plus replicated
  tensors), with 0 OOM and 0 growth.  The loader's own evidence agrees: `Meta() model buffer size =
  14893.51 MiB`, i.e. the larger device's share of a 25972 MiB model at ratio 0.5628.  The **single-device**
  case that block 07 creates (no Meta wrapper to expand) also engages the tensor path correctly:
  `tensor split: 1 devices, estimated use 26503 MiB vs. 31376 MiB target`, no changes needed, 19.2 t/s,
  0 OOM and 0 growth - which is why the change needs neither block 07 nor a Meta device to exist.
* `scripts/validate-set.sh` green (strict 16/16 `git am` on a fresh `ebbb18522`, applied tree `8a80535e5`),
  and patches `0006`-`0015` changed (block 06 in content, 07-15 in their `From`/`index` lines only).

**Carried limitations** (unchanged by the promotion, from `archive/work/tensor-fit-fix/README.md`): the
estimate is deliberately conservative rather than exact (the Meta row's `context`/`compute` are per-device
maxima counted once per device, and replicated tensors are under-counted), with the default 1 GiB/device
margin absorbing the measured ~0.4 GiB/device error on 27B Q8_0; block 15's FA **prefill staging arena**
...
**Release** `v16-84e76d8a2-r6`, tip `b3c3051a72df21f600f5ae13b244c8212210ca2e`, net tree
`504894e61e17c6616b54871abee9fb23beda38bd`.  Only **block 15** changed; blocks 00-14 are
byte-identical (not amended, so their patch files are unchanged, `From <sha>` included).

**The change.**  `ggml_cuda_fattn_kv_native_bf16_enabled()` (`fattn-common.cuh`) now returns
`ggml_cuda_fattn_kv_native_policy() != FATTN_KV_NATIVE_OFF` instead of `== FATTN_KV_NATIVE_ON`, so the
bf16 native arm follows the same auto policy as q8_0/q4_0/q4_1/q5_0/q5_1/iq4_nl:
`GGML_CUDA_FA_KV_NATIVE` unset (or `=1`) enables it, `=0` disables every native arm.  The comment
block above the policy is updated to drop the "bf16 stays opt-in" rationale.

**Why now.**  Block 15's V5 bf16 native arm was left opt-in on 2026-09-15 because it "only removes the
MMA scratch, at ~1-2 % prefill with no equivalent decode win".  r5 removed that second clause: the
band gate follows `ggml_cuda_fattn_kv_native_type`, so native bf16 is exactly what puts a bf16 cache on
the RDNA4 GQA-6 decode/verify band.  The maintainer called it on 2026-09-25, noting the beta prefill
boosts being integrated soon outweigh the small prefill cost.

**Measured (gfx1201 R9700, 27B qwen35 head 256 GQA 6, bf16 KV).**  Op level kv 16384,
`test-backend-ops perf`, default (band) vs `GGML_CUDA_FA_KV_NATIVE=0` (staged tile), us/run at
`n_q` 1/3/5/8: 145/164/264/285 vs 104/276/428/655.  End to end, 27B UD-Q4_K_XL `draft-mtp` n3 at ~30k,
...
## 2026-09-25 block-15 amendment (r5): f16 (and bf16) on the RDNA4 GQA-6 FA band (issue #45 follow-up)

**Release** `v16-84e76d8a2-r5`, tip `62eaaec3e41bbefeda2f3625ecd6e6f7e814e2f0`, net tree
`de86c5e11f8dbebedec42be16c00cda7f68853a2`.  Only **block 15** changed; blocks 00-14 are byte-identical
(their patch files are unchanged, `From <sha>` included, because they were not amended).

**The report (issue #45 comment, @DanoPTT).**  r4's band was quantized-KV-only: the gate asked
`ggml_cuda_fattn_kv_native_type(K) != NONE`, and f16 has no native read, so f16 stayed on the tile
kernel.  But at verify widths the tile kernel is not DRAM-bound.  The F1 purity pin
(`fattn-tile.cuh`, block 08: `cols_per_block = ncols2 > 2 ? ncols2 : 2`) makes the whole `n_q <= 8` band
use the `n_q = 1` config, and with GQA 6 (`ncols2 = 2`) every query row re-reads and re-stages every
K/V element three times (once per head pair).  On gfx1201, f16 kv 102400: `n_q = 1` reads K/V at
~604 GB/s, `n_q = 4` only ~180 GB/s, so the verify width was ~50 % slower than an older tree that used
a wider tile config -- but that older tree did not keep decode == verify.  The reporter asked whether
the band could cover f16/bf16.

**The fix.**  The band gate now also accepts f16 (`K->type == F16 && V->type == F16`); the
native-quantized set is unchanged.  bf16 was already covered through its opt-in native arm
(`kv_native == BF16`), and is deliberately **not** banded while staged: the staged f16 copy is produced
by a whole-cache conversion that dominates (measured 3x the tile cost), so the valuable bf16 path is
native bf16.  The band's config is now chosen per K/V element size, because the two classes have
different optima:

* **native-quantized** (`q8_0`/`q4_0`/`q4_1`/`q5_0`/`q5_1`/`iq4_nl`): `ncols1 = 4`, `P = nsm`
  (**unchanged r4 tuning**) -- the dequantization dominates and hides the columns an `n_q = 1` decode
  leaves unused.
* **2-byte** (`f16`, and bf16 when its native arm is on): `ncols1 = 2`, `P = max(2, 3*nsm/4)`
  (24 on gfx1201, `nsm` = 32).  With no dequantization the unused columns are a large fraction of the
  `n_q = 1` cost (gfx1201 kv 102400: `n_q` 1 747 vs 967 us, `n_q` 4 861 vs 970, a 2 vs 4 column tile),
  and the lighter per-iteration work makes the `P`-partials fixup dominate, so a smaller `P` wins.

`ggml_cuda_fattn_band_wmma_applies` is still shared by the chooser, the ncols dispatcher and
`launch_fattn`; only the helpers now take the op tensor so they can key off `K->type`. 
...
invariant: every K/V type has exactly one config for the whole band.
...
flat.  The bf16 native default is left opt-in by r5 and **flipped ON by r6** (see above and the
block-15 r6 section below).

**Gates.**  `test-backend-ops -o FLASH_ATTN_EXT` **6340/6340 on ROCm0** (the 389-case qwen35 subset is
green on the final build); f16 `--spec-type none == draft-mtp` byte-identical at ~5k
(`3c31df680ac1`) and ~30k (`32f533498f84`); q8_0 identical at ~5k (`50ca5b987f85`), so the quantized
band is unregressed (op level within noise: q8_0 kv 16384 147/148/222/227 -> 150/152/227/232 us).

## 2026-09-25 block-15 amendment (r4): the RDNA4 GQA-6 decode/verify FA band (issue #45)

**Release** `v16-84e76d8a2-r4`, tip `f744c11e6ee452d7cdc2786a9b9290b62c8fc5be`, net tree
`5938da09d294a01e0862c2d561b0c7ca154de90a`.  Only **block 15** changed; blocks 00-14 differ only in
their `From <sha>` lines (and the series count, which does not change here).  Folded into block 15
because the band fast path uses the block's native-KV descriptor (`fattn_kv_native_t`) and derived-mask
symbols.

**The bug (issue #45, @overdoingism).**  At head 256 with GQA 6 (Qwen3.8-27B: 24 Q / 4 KV heads) the
tile kernel can only fold `ncols2 = 2` -- its `ncols2` must divide the GQA ratio -- so the whole
`n_q <= 8` decode/verify band fetched and dequantized every K/V element once per head pair (three times
per query row).  On gfx1201 a q8_0 cache at `n_q = 1`, kv 102400 reads K/V at ~276 GB/s against
~615 GB/s for f16, so the path is instruction-issue bound and the fused fold is the lever; Vulkan folds
all six heads into one workgroup and measured ~1.7x faster there.

**The fix.**  The whole band (`n_q = 1` included) routes to the existing WMMA instances
`(256, ncols1 4|2, ncols2 8)` -- no new instantiations -- and the KV is split **round-robin** over a
fixed `P = nsm` blocks per output tile (`launch_fattn`, `GGML_HIP_FA_BAND_WMMA_SPLIT` override for
tuning).  `P` depends on neither `n_q` nor the KV length, so decode and every verify width accumulate
the same partials in the same order; the reporter's first, contiguous stream-k version moved its split
points with the 256-padded KV length and diverged at KV position 18944 = 74x256, which is why the
round-robin form is part of the contract, not a detail.  `blockIdx.y` is the round-robin lane and the
existing uniform fixup (`flash_attn_stream_k_fixup_uniform`) combines the partials; the fixup-buffer
indexing is written in terms of `gridDim.y` so the `gridDim.y == 1` stream-k path is unchanged.

**One predicate, three call sites.**  `ggml_cuda_fattn_band_wmma_applies` (self-contained, in
`fattn-common.cuh`) is asked by the kernel chooser (`ggml_cuda_get_best_fattn_kernel`), the ncols
dispatcher (`ggml_cuda_flash_attn_ext_mma_f16_switch_ncols2`) and `launch_fattn`, and `launch_fattn`
additionally requires the `ncols2 == 8` template parameter.  A disagreement can therefore never launch
the 2-D grid into a kernel whose band fast path is compiled out and whose stream-k split is
query-width dependent.  `GGML_CUDA_FA_WMMA_256=0` and `GGML_CUDA_FA_WMMA_MAX_HEAD<256` disable the
band exactly like the generic head>128 WMMA band, so the existing escape hatches stay complete.

**Type coverage is the native predicate, not q8_0.**  The reporter measured q8_0 (their cache type),
but the tile kernel's 3x re-fetch is a property of every instruction-issue-bound quantized type, so the
gate follows `ggml_cuda_fattn_kv_native_type`: **q8_0, q4_0, q4_1, q5_0, q5_1 and iq4_nl** all take the
band.  f16 has no native read (`NONE`) and is DRAM-bound, so it stays on tile; bf16 stays on tile while
its native arm is opt-in.  Default **ON** (maintainer policy); `GGML_HIP_FA_BAND_WMMA=0` opts out, `=2`
selects `ncols1 = 2`.

**Measurements (gfx1201 R9700, 27B qwen35 head 256 GQA 6, q8_0 KV, `test-backend-ops perf` @ kv 16384,
tile -> band us/run).**  `n_q` 3: 326 -> 150; `n_q` 5: 524 -> 225; `n_q` 8: 814 -> 230.  `n_q` 1:
q8_0 141 -> 148, q5_1 178 -> 198 (the single-column cost), while q4_0 165 -> 156, q4_1 174 -> 161 and
iq4_nl 235 -> 216 are faster.  End to end, 27B UD-Q4_K_XL `draft-mtp n3` at ~40k ctx, band off -> on:
1 GPU layer 43.3 -> 51.3, 2 GPU tensor 67.9 -> 76.6, 3 GPU tensor 77.6 -> 88.8 t/s (**+13-18 %**).
Prefill is flat (`llama-bench` pp512/2048/4096 2049/2060/2056 vs 2056/2059/2054 t/s on 3-GPU tensor),
because the band is gated to `n_q <= 8` by construction.

**Gates.**  `test-backend-ops -o FLASH_ATTN_EXT` **6340/6340 on ROCm0**, including 389 new qwen35 GQA-6
cases across all eight KV types and both layouts; `plain == draft-mtp` byte-identical on all eight KV
types at ~40k ctx, 3-GPU tensor (acceptance 0.83-0.86); q8_0/q4_0 text identical across
`--spec-draft-n-max 3/5/7` (verify widths 4/6/8); 1 GPU layer, 2/3 GPU tensor (including the uneven
4-KV-head 2/1/1 split) and 3 GPU layer all pure.  New eval + perf cases in `tests/test-backend-ops.cpp`.

## 2026-09-20 block-15 amendment (r11): the compute reserve accounts for the reachable (packed) kq mask (issue #42)
...
content-identical to r10 (their patch files are byte-identical, `From <sha>` included).

**The bug (issue #42).**  V3's derived kq mask is a per-*batch* decision, and the packed mask that a
batch it cannot serve allocates is `n_kv*n_tokens*2` bytes -- hundreds of MiB at depth.  `sched_reserve()`
measured with the derived form **on**, so the reserved compute buffer did not contain that mask: the first
non-derivable batch at depth had to *grow* the buffer mid-run, and under `--fit on --fit-target 256` that
growth fails with `cudaMalloc failed: out of memory`, the request dying with `failed to process mtmd
chunk`.  The failed reserve also left `ggml_gallocr` describing buffers it no longer had, so the **next**
request tripped `GGML_ASSERT(... addr + alloc_size <= base + size)` in `ggml_backend_tensor_alloc` -- the
hard crash in the issue.

**Reproduced** on gfx1100 (RX 7900 XTX, 24 GiB) with the reporter's exact model and settings
(`ukisai/Swift-Qwen3.8-27B-GGUF:Q4_K_S` = `qwen35`, M-RoPE + mmproj; `--fit on --fit-target 256 -b 2048
-ub 512 -fa on -ctk q8_0 -ctv q8_0 --spec-type draft-mtp --spec-draft-n-max 3 -np 1 --cache-ram 16384
--cache-reuse 256`): ~152k-token prefill, then a request appending an image at depth.  Pre-fix the reserve
is **122.80 MiB** and the 800-token image batch grows it to **271.53 MiB** -- the delta is exactly the
packed mask, `152320 x 512 x 2` (152208 rounded up to the 256-cell padding), so the image batch is the one
that cannot be derived.  HTTP 500, then an assert on the following request.

**The fix.**  `llama_context::graph_reserve()` takes an explicit `packed_kq_mask` argument; when set, the
measure graph is built with `cparams.kq_mask_derived = false`, so the reserved buffers contain the packed
mask at its worst case and the runtime growth cannot happen.  Which reserves pass it is decided by the new
`llama_context::kq_mask_packed_reachable()`, and that predicate is the interesting part: the mask has to be
reserved only where a batch the derived form cannot serve is actually **reachable**:

* **2-D (M-RoPE) batches** -- an image/audio chunk decode carries 2-D positions (mtmd), which only an
  M-RoPE model can produce (`llama_batch::is_pos_2d()` is `n_pos >= 3`; `n_pos_per_embd()` is 4 only for
  MROPE/IMROPE).  This is the reporter's case.  The packed mask's `ext`-based causal clause
  (`set_input_kq_mask_impl`: when `p0 == p1`, drop if `p0_ext.is_2d_gt(p1_x, p1_y)`) is exactly what the
  derived form cannot express, so the rejection is necessary, not conservative.
* **multi-sequence batches** (`ubatch.n_seqs_unq != 1`), e.g. `--kv-unified` with several slots --
  `kq_mask_derivable()` rejects them because the derived cell encoding is single-sequence.
* **alibi**, which encodes a value rather than a predicate (belt-and-braces -- alibi already disables the
  derived form globally through the probe).

Everything else is already covered *without* the predicate, which is what keeps the cost narrow:

* the `allow_derived == false` builders (MLA / lightning indexer / MSA) never take the derived branch, so
  their reserve graph already holds the packed mask -- no arch list is needed;
* alibi, a multi-stream KV cache and a disabled flash attention disable the derived form globally
  (`resolve_fused_ops()`'s probe), so those reserves are packed anyway;
* the `n_kv % 256` gate is not a runtime variable: `llama_kv_cache::get_n_kv()` pads to at least 256 cells
  (`GGML_PAD(cells.used_max_p1(), max(n_pad, 256))`), so the stride condition always holds.  Verified
  empirically on gfx1100 with a text-only multi-turn append at an off-stride position: **every** prefill
  graph logged DERIVED (380 DERIVED / 20 PACKED, and every PACKED one was `n_tokens <= 8`, whose mask is a
  few MiB and fits the reserved slack);
* a `n_tokens <= 8` batch takes a packed mask, but it is tiny (the same was true before the derived form
  existed).

The predicate is the one place that enumerates the reasons `kq_mask_derivable()` can reject a *large*
batch, so a new reason added there has to be added here too; the `ggml-alloc` invalidation and the
compute-buffer growth log below are the safety net if that is ever missed.  The fused-op support probes
deliberately keep the derived form (the `LLM_FUSED_OP_FLASH_ATTN_DERIVED` probe requires the derived node
to be *present* in its graph), and the public `llama_graph_reserve()` ext API keeps the old default.

**Cost, measured.**  The reporter's M-RoPE model pays, as it must: `ROCm0` **122.80 -> 258.02 MiB**,
`ROCm_Host` **20.80 -> 210.02 MiB**, and `--fit` solves for `n_ctx` **203520 -> 194560** (**-8960 tokens,
-4.4 %** of context on the 24 GiB card).  The common case does **not** pay: gemma4-E4B (not M-RoPE, single
sequence) keeps the derived reserve at **113.94 MiB**, against **146.80 MiB** with the derived form forced
off (`LLAMA_KQ_MASK_DERIVED=0`) -- i.e. V3's *reserved*-memory win stands for every configuration where the
packed mask is unreachable.  Throughput is untouched anywhere (the reserve is not on the compute path):
same-seed prompt eval **963.65 -> 963.57 t/s**, decode **38.81 -> 38.71 t/s**, 152k-token prefill
**267.7 -> 267.9 s**.

**Allocator hardening (same release, independent).**  `ggml_gallocr_reserve_n_impl()` now clears a new
`galloc->layout_valid` when one of the buffers cannot be allocated, and `ggml_gallocr_needs_realloc()`
forces a fresh reserve whenever it is clear.  Before this, the failure left `node_allocs`/`leaf_allocs`
describing the new (larger) graph while the failed buffer was NULL and the not-yet-processed ones still had
their old size, so the next `ggml_gallocr_alloc_graph()` reused that layout and either dereferenced a NULL
vbuffer or pointed a tensor past the end of its buffer (the assert above).  With the flag, an out-of-memory
becomes a clean `GGML_STATUS_ALLOC_FAILED` (`process_ubatch: failed to allocate graph`, the request fails,
the server survives), and a retry after the memory is freed can succeed.  No steady-state cost.  The
buffer-growth message was promoted from `GGML_LOG_DEBUG` (compiled out in Release, which made a failed
growth unattributable) to `GGML_LOG_INFO`.

**Validation.**  Pre-fix on fingon: image-at-depth -> `allocating 271.53 MiB ... cudaMalloc failed: out of
memory` + `failed to process mtmd chunk` (HTTP 500).  Post-fix the identical run logs **zero**
growth/OOM/assert events, the image decodes in 1281 ms, HTTP 200, and text-only and image follow-up
requests both succeed -- the allocator is not left corrupted.  Same-seed greedy output is
**byte-identical** pre- vs post-fix on both the derived (text) and packed/2-D (image) paths
(`620cbe029ab2679408b591c73bde730a154a091361a80611e63374d850a64489`, with `-c 32768 --fit off` pinned so
the changed fit cannot confound it).  `scripts/validate-set.sh` green (strict 16/16 `git am` on a fresh
`ebbb18522`, applied tree `865ded736`), and only patch `0015` changed.

## 2026-09-20 block-11 amendment (r10): the pre-fill test reads the real token count, and the HIP exec is re-instantiated instead of updated

**Release** `v16-ebbb18522-r10`, canonical tip `385e0c77cbc34a01707b2efc25adb684c0dcbbc1`, net tree
`9f9602e6e5751ca1e065b80ec3764fdfe6ca6eba`.  Only **block 11** changed (issue #41); blocks 00-10 are
content-identical to r9 (their `From <sha>`/`index` lines move with the rebuild) and blocks 12-15 are
unchanged in content but carry new commit SHAs because they sit above block 11.

**The bug.**  Block 11's pre-fill test was `cgraph->nodes[0]->ne[1] > 1`.  With expert offload
(`-ncmoe`) the scheduler splits the graph around the CPU-resident experts, so a **one-token** decode
split routinely starts on an expert-path tensor `[n_ff, n_expert_used, 1]` and `ne[1] ==
n_expert_used` (10) even at one token.  Every MoE decode split was therefore classified as pre-fill,
`use_cuda_graph` was forced false, and decode never captured, warmed up or replayed a HIP graph.  This
is in the delivery as shipped and needs no upstream expert-cache PR (the reporter's #27861 stash is
incidental).  It also explains the r9 observation that `GGML_CUDA_DISABLE_GRAPHS=1` made no difference
to MoE decode.

**The fix.**  `ggml_cuda_graph_is_multi_token()` reads the token count from the first op that actually
carries it: `MUL_MAT_ID`'s `ne[2]` (the result is `[n_out, n_expert_used, n_tokens]`), or a weight
`MUL_MAT`'s `src1->ne[1]` (the result is `[src0->ne[1], src1->ne[1], …]`).  The `MUL_MAT` arm requires a
constant, unbatched weight (`src0` op `NONE`, `ne[2] == 1`) so attention-score matmuls do not mislead
it, and a split with no weight matmul falls back to the old `nodes[0]->ne[1]` test.

**Measured** on the reporter's exact model (gfx1201, Qwen3.8-Flash-Next UD-Q4_K_XL, `-ngl 99 -ncmoe 48
-b 2048 -ub 2048 -c 8192 -ctk q8_0 -ctv q8_0`, one GPU): the r9 tree logged **0** warmups / **0**
replays for the whole run; the amended block 11 logs **50** / **687** in a 16-token decode and takes
`tg` **10.6 -> 12.8 t/s** (~+20 %; a second pair 10.3 -> 12.9).  Greedy output is byte-identical old
vs new, pre-fill is unchanged (18.7 vs 18.6 t/s), and a dense 4B run is unchanged (1 warmup / 22
replays, same `tg`, same text).

**The leak guard that must travel with it.**  Once decode recaptures regularly, `hipGraphExecUpdate`
leaks device memory on ROCm <= 10.0: the driver's `GraphKernelArgManager` bump-allocates a kernarg slot
per update and only reclaims slots on `hipGraphExecDestroy` (ROCm/rocm-systems#10713; the driver fix,
PR #11434, is still unmerged).  `ggml_cuda_graph_update_executable()` now destroys and re-instantiates
the exec on HIP rather than updating it — this runs only on the recapture path — and
`GGML_HIP_GRAPH_FORCE_UPDATE=1` keeps the update path for a ROCm carrying the driver fix.  The leak
itself could not be independently measured here because ROCm 7.14.1's `rocm-smi`/`amd-smi` counters on
this host report ~57 MB while the process holds ~8.2 GB (blind to the driver pool); the workaround is
the reporter's validated one and a 1500-token soak ran clean with it.

See `../WORKLOG.md` 2026-09-20 (r10) and the block-11 commit message.
...
**Release** `v16-ebbb18522-r5`, canonical tip `d82d07a312dbc3d5df945b36cbb893784f0f31cf`, net tree
`06b89471790c52d7afa32f75755fb1b3b22edada`.  Block 04 is amended with **one value (576 -> 256) plus a
comment** in `ggml/src/ggml-cuda/fattn.cu`; blocks 00-03 and 05-15 are content-identical (the rebase
replayed block 08 over the amended comment; the net tree differs from r4's `b84b1783f...` in that one
hunk).

**Why.**  The 2026-09-18 r4 amendment fixed the *tensor-split* half of the RDNA4-tuned 2026-09-14
change but left the other half: the 2026-09-14 config transfer had also overwritten the **RDNA3_0**
side of `ggml_cuda_fattn_mma_get_config_rdna` (heads 320/512/576) with upstream #28102's
**RDNA4/gfx1201** rows, and lifted the RDNA3_0 WMMA head cap from stock's 256 to 576.  Neither the
rows nor the lifted cap was re-validated on gfx1100, and gfx1100 then took the WMMA kernel for head
512 where stock takes the tile kernel.

**Measured on a single RX 7900 XTX (gfx1100, ROCm 7.14), gemma-4-26B-A4B UD-Q4_K_XL (head 512,
kv-heavy), `pp2048 @ d98304`, 2-3 reps:**
...
So the r4 delivery was **-14.4 % (q8_0) / -22.9 % (bf16)** below the tile kernel at this point.
Restoring the upstream base head-512 row recovers most of q8_0 but still leaves bf16 9 % below tile,
so the regression is the WMMA *kernel choice* for head 512 on RDNA3_0, not just the row values.  Tile
is exactly what stock `ebbb18522` chooses (`Q->ne[0] <= 256`), and head 256 itself is emphatically
not a candidate for tile: forcing tile on the 9B dense model (head 256, q8_0) costs `pp2048`
1780 -> 1232 t/s at d65536 and 1407 -> 929 t/s at d98304 (**+44-52 % for WMMA**), so the cap must
land at 256, not lower.

**Fix.**  `ggml_cuda_get_best_fattn_kernel`:
...
RDNA3_0 now takes the tile kernel for heads > 256 (stock behaviour); RDNA4 (576) and RDNA3_5 (320) are
unchanged.  `GGML_CUDA_FA_WMMA_MAX_HEAD` still overrides the cap for experiments, and
`GGML_CUDA_FA_WMMA_256=0` still forces the tile kernel entirely.

**Verification (gfx1100, single RX 7900 XTX).**  `test-backend-ops -o FLASH_ATTN_EXT` **5952/5952**.
Gemma-4-26B-A4B `pp2048` at d0 / d65536 / d98304 q8_0 **3635.7 / 1051.3 / 778.2 t/s** (was
3480 / 906 / 661 pre-fix), bf16 **3627.7 / 1139.6 / 853.3** (was — / 895 / 656), f16
**3660.4 / 1112.8 / 818.2**.  Dense 9B (head 256) q8_0 `pp2048 @ d98304` **1404.7 t/s** (WMMA still
in use; no regression).  Same-seed greedy output coherent.  The change only touches head > 256
*prefill* (`n_q > 8`); the decode/verify band (`n_q <= 8`) already took the tile kernel, so the
greedy-purity band invariant is untouched.

**Related finding (kept as a trade, not changed).**  On gfx1100 the block-15 q8_0 native arm costs
~5 % of gemma-4-26B-A4B head-512 deep prefill (`GGML_CUDA_FA_KV_NATIVE=0` recovers 773 -> 813 t/s at
d98304, matching stock 810) but buys **+44 %** decode at d65536 (tg128 109.9 -> 76.4 with native off;
stock 71.5), so it stays on.  The dense head-256 model shows no prefill difference (1405.9 vs 1404.4).
A future fix would keep the native decode path but restore the node-scratch F16 staging for prefill on
RDNA3_0; not attempted here.

**Files.**  `ggml/src/ggml-cuda/fattn.cu` only.  `scripts/validate-set.sh` green (strict 16/16 `git am`
on a fresh `ebbb18522` tarball, applied tree == `06b8947179...`).
...
| `0011` | skip CUDA graphs for multi-token PRE-FILL | **amended 2026-09-20 (r10, issue #41): the pre-fill test is `ggml_cuda_graph_is_multi_token()`, not `nodes[0]->ne[1]`** — a split-MoE one-token decode split starts on an expert tensor `[n_ff, n_expert_used, 1]` and was misclassified as pre-fill, so with `-ncmoe` decode never replayed a HIP graph; the helper reads `MUL_MAT_ID`'s `ne[2]` / a weight `MUL_MAT`'s `src1->ne[1]`, and `ggml_cuda_graph_update_executable()` destroys/re-instantiates the exec on HIP (leak-free under ROCm <= 10.0; `GGML_HIP_GRAPH_FORCE_UPDATE=1` opt-out) — see the 2026-09-20 block-11 amendment section below. |
...
V3 (`LLAMA_KQ_MASK_DERIVED`, default 1) computes the attention mask in-kernel from compact per-cell
state instead of materialising the `n_kv x n_q` f16 mask, which saves `n_ubatch x n_ctx x 2` bytes of
compute buffer (measured on the 9B at `-c 98304`, ub 512: **184.02 -> 88.39 MiB** device and
**112.02 -> 16.40 MiB** host).  Issue #30's second report (@a-n-t-0, 2x RX 7900 XTX / gfx1100, 27B
...
in the kernel, not a lookup:

* the derived branch did **one cell per thread step with a scalar `half` store** -- twice the
  iterations and unvectorised shared stores of the packed fallback, for strictly *less* global
  traffic; and
* it re-read `cell_pos` for **every query row**, even though a cell's position does not depend on the
  row (the packed path does re-read its mask per row, the derived path must not re-read `cell_pos`).

**The fix** reshapes the derived branch to mirror the packed fallback: two cells per thread step with
one `half2` store, and the `cell_pos` load hoisted out of the `j1` loop.  `nbatch_fa` is always a
multiple of 32 and `<= 256` (config-table `static_assert`), so the even `i` never makes `i + 1` leave
the tile and no tail handling is needed.  The mask values are unchanged.

**Verification.**  PP512, `-r 3`, `derived=1` vs `=0`:

| config | before | after |
|---|---|---|
| gfx1201 27B 2GPU layer @98k | -5.96 % | -1.62 % |
| gfx1201 9B 1GPU @32k / @98k | +1.64 / +2.57 % | +1.89 / **+3.49 %** |
| gfx1151 9B 1GPU @32k / @64k | -0.53 / -0.86 % | -0.24 / -0.86 % |
...
bf16 off [bf16 flipped to **on** in r6, 2026-09-25]; `=1` force all on; `=0` force the F16-staging path), and q4_0 has a native arm
...
(This block is *not* where r12 landed: the `--fit` for `-sm tensor` change went to **block 06**, the
general system-operations bucket, because it is dependency-free and fits no other block.  See the
2026-09-21 block-06 (r12) amendment section above.)

**Amended 2026-09-20 (r11, issue #42)**: `sched_reserve()` now measures with the packed kq mask when a
batch V3's derived form cannot serve is *reachable* in the context (the new `kq_mask_packed_reachable()`:
2-D M-RoPE or multi-sequence), so such a batch can no longer force a mid-run compute-buffer growth that
fails under `--fit`.  Where the packed mask is unreachable the reserve is unchanged, so V3's
**reserved**-memory win (`n_ubatch x n_ctx x 2`) stands for M-RoPE-free single-sequence models (gemma4-E4B
113.94 MiB) and is only given up where the mask can actually appear (the reporter's M-RoPE 27B: 122.80 ->
258.02 MiB) — see the 2026-09-20 block-15 (r11) amendment section above.
