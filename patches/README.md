...
reduction and an `-ngl` binary search, with an explicit `-c` never overridden.  Block 15 is the home
because it is the last block touching `ggml-backend-meta.cpp` and the change depends on no block.  It was
re-validated on r11 before promotion: the default cases reproduce the 2026-09-18 record exactly, the
`-ngl`-reduction cases are more conservative (the fit now sizes for the packed mask r11 restored for
M-RoPE), seven end-to-end loads generate with zero out-of-memory and zero compute-buffer growth, and the
same-seed gate is byte-identical.  See the 2026-09-21 block-15 (r12) amendment section below.  **r11
...
## 2026-09-21 block-15 amendment (r12): `--fit` supports `-sm tensor` (promoted from `beta/tensor-fit-fix/`)
...
content-identical to r11 (their patch files are byte-identical).  The campaign record moved from
`../beta/tensor-fit-fix/` to `../archive/work/tensor-fit-fix/` with this promotion.
...
so the change depends on no block.  Its only overlap is `ggml/src/ggml-backend-meta.cpp`, which blocks
09/14/15 touch - and block 15 touches it last, at lines ~795/~1032 versus this change's ~83/~196.  Block
15 is therefore the home both as "the last block that touches the file" and so that the de-static change
can never be invalidated by a later block.  (Block 06 is the host-buffer revert for discrete GPUs and
never touches that file.  The alternative home is `upstream/` as an `UPSTREAM-PR-*` candidate, which
remains worth doing: the change is generic llama.cpp.)
...
**Amended 2026-09-21 (r12)**: `--fit` now supports `-sm tensor`.  Upstream threw
`llama_params_fit is not implemented for SPLIT_MODE_TENSOR` and `common_fit_params()` swallowed the
exception, so the default-**on** `--fit` was a silent no-op under tensor split.  The Meta device's
accessors are exposed and `common/fit.cpp` gained a dedicated tensor path (per-device targets from
`--fit-target`, proportional or honoured `-ts`, then auto-`n_ctx` and an `-ngl` binary search).  See the
2026-09-21 block-15 (r12) amendment section above; the campaign record is
`../archive/work/tensor-fit-fix/`.
