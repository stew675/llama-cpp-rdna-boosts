...
## 2026-09-12 block-14 amendment (sixth): the QSA prefill crossover + the device-query arm gate

Two changes to `src/models/qwen4exp.cpp`, both resolving TODO item 9.  Full measurements, repro and
the gate table: `../wip/strix-halo/qsa-item9/RECORD-2026-09-12-qsa-prefill-crossover.md`.

**1. The QSA prefill arm is depth-configurable and split-tuned (`qsa_dense_prefill_until`).**  The
arch policy had one axis - the *decode* crossover (`qsa_dense_decode_until`: 64K on gfx1151, dense
always on gfx1201) - and prefill took the indexer top-k selection from the `indexer_top_k + r - 1`
(2051) shortcut width up, unconditionally.  A prefill ubatch (`n_tokens > QSA_DECODE_BAND`) whose
`n_kv` is still below the new threshold now attends dense, storing the indexer keys exactly as the
shortcut arm does, so the sparse path takes over seamlessly above it.  Defaults (all overridable
with `LLAMA_QSA_DENSE_PREFILL_UNTIL`, `0` = the pre-amendment regime):

| config | default | evidence |
|---|---|---|
| gfx1151 (Strix Halo, 1 GPU) | **8192** | measured, below |
| tensor split (Meta device, >= 2 GPUs) | **16384** | the recorded 2026-09-07 3x R9700 table |
| every other arch | 0 | unmeasured (keeps the pre-amendment regime) |

Measured on gfx1151, whole-prompt pp, IQ4_XS, f16 KV, `b=ub=2048`, r2, interleaved in one session
(the new default against `=0`):

| pp | default `8192` | old regime | delta |
|---|---|---|---|
| 4096 | 751.9 / 742.1 | 724.0 / 723.2 | **+3.2 %** |
| 8192 | 751.7 / 750.2 | 731.8 / 730.3 | **+2.7 %** |
| 16384 | 742.0 / 741.3 | 731.7 / 731.2 | **+1.4 %** |
| 32768 | 711.8 / 712.3 | 707.7 / 707.4 | **+0.6 %** |

The crossover itself (all-dense vs all-sparse, same session): pp4096 754.6/740.8 (dense +2.4 %),
pp8192 747.3/737.8 (dense +1.6 %), pp16384 712.4/735.1 (sparse +3.0 %), pp32768 604.5/709.5 (sparse
+17.4 %) - ~8-16K, materially the same place the recorded tensor table puts it.  The policy is a
strict win at *every* measured pp because the arm is **self-limiting**: it only ever applies to
prefill chunks whose `n_kv` is still below the threshold, i.e. the shallow chunks, and those are the
ones that measured faster dense - so it never repays the deep chunks' sparse advantage.  Note the
recorded 2026-09-07 tables compare `LLAMA_QSA_OFF=1` (no indexer store at all), so they *bound* this
arm rather than equal it; the tensor default is table-derived because the 1-GPU dev box cannot
re-measure a tensor split, and it is the same policy seam as the decode half (landed as a default
from that same table on 2026-09-07).

It is a **quality** win too, which is why it is not just a speed knob: the selection is a lossy
approximation.  Perplexity (8x4096, f16, `--chunks 8 -c 4096 -ub 512`) reads **23.2727 +/- 0.727**
for the new default, *identical* to `LLAMA_QSA_OFF=1` (the no-indexer full-dense reference),
against **24.7142 +/- 0.792** for the old regime - below the crossover the old default was using an
approximation that cost 6.2 % PPL *and* ran slower.  Documented caveat: the oracle text
(`/tmp/qa-text.txt`) is absent on this box, so the sparse-vs-its-own-dense-masked parity claim can
only be carried by the 3-GPU box (see the record).
...
ROCm0/gfx1151 with `wip/strix-halo/qsa-item9/qsa-support-probe.cpp`: **0 mismatches** against the
...
`86b6cce726b0f0f2f3935781ed782659529b38fe`; `test-backend-ops -o FLASH_ATTN_QSA` **22/22** and
`-o FLASH_ATTN_EXT` pass; q8_0 KV text is pure across the whole band (`plain` == `n_max 1/2/3/5/7`
== `93deb49ca115`, 685 chars) and f16 pure both regimes; the `mstep` width probe is 0 mismatches at
W=4 in both regimes, and W=8's 38 mismatches have **identical position lists** in both (pre-existing,
not from this change); MTP `n_max 3` 28.3 t/s vs plain 24.5 (+16 %) with pos-1 acceptance 0.667.
...
`../wip/kv-quant-purity-followups/HANDOVER-2026-09-11-f3-step2-iq4_nl.md`).  Before this, `iq4_nl` produced **no
...
* **Block 15's dense masked arm (`LLAMA_QSA_SPARSE_FA=0`) is broken for every KV type** (PPL ~1.05) — found by the
  eighth beta re-cut; see `../beta/block-15-campaign-wins/BETA-TESTING.md` §4c.  The delivery itself is unaffected
