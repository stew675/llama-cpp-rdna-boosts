...
5. **Throughput, interleaved same-binary A/B** (llama-bench, `-fa on`, the `--output csv` parser in
   `archive/work/qwen4exp/qsa-memory/tools/` is not csv-quote-safe — use a real CSV reader):
...
* delivery repo: `patches/` = 15 patches (block 15 = the campaign wins), `upstream/` = 4 candidates,
  `archive/work/block-15-campaign-wins/` = the beta record; working tree clean.
...
fork point; four files, +171 net lines; `archive/work/arch-independent-memory/patches/0007-v5-native-bf16-kv.patch`
is the snapshot):
