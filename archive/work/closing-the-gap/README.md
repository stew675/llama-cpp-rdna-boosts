# `archive/work/closing-the-gap/` — the prefill/decode gap to the other solution's `strix-halo`

**Status: WIP planning record. Not part of the delivery.**  Started 2026-09-20 (analysis on gfx1151),
moved here and updated 2026-09-21.  **The campaign's code was CONSOLIDATED into
[`../../archive/work/mmb-general/`](../../archive/work/mmb-general/) on 2026-09-25** (28-patch set, r13-based,
applied tree `468c6496…`); the `patches/` tree below is kept as the historical export — see
...
| [`gfx1201-closing.md`](gfx1201-closing.md) | **the live gfx1201 (3× R9700) brief: the remaining open items** (OP-1 the MTP CPU-spin fix — runtime half DONE as `0029`, structural/1.4 open; OP-2 the throughput re-baseline DONE; OP-3 the `0028` follow-ups — matrix DONE, …) + the operational apply/build/gate instructions for a fresh session. |
| [`gfx1201-closed.md`](gfx1201-closed.md) | **the gfx1201 closed-items archive** (pre-split snapshot): the session log, the gate results, the per-patch verdict table and the DONE work items (§12.1-§12.3, the `0016` port, the `0028` cliff fix). |
| [`gfx1100-closing.md`](gfx1100-closing.md) | **hand-off brief for the gfx1100 (single RX 7900 XTX, 24 GiB) box:** the same stack on RDNA3_0 — prove the default-ON MMB recovers the S5-S10 wins, validate the arch-neutral/MoE additions, compile/oracle-check the qwen4exp-only patches (no model fits), and port the RDNA3_5-only WMMA kernels. |
...
| [`2026-09-23-mtp-sparse-default-on.md`](2026-09-23-mtp-sparse-default-on.md) | **Session 12 — sparse MTP draft prefill default ON, `patches/0026`.**  The old default-OFF reason (HC16 F32-elision made the sparse draft's depth text diverge) was fixed by `0023`; flip the switch and keep `LLAMA_MTP_SPARSE=0` as the opt-out.  A/B: pp150K **937.1 -> 1007.9 t/s (+7.6 %)**, pp16K −0.3 %, 8K decode parity, 40K text byte-identical; acceptance 0.85035 unchanged; depth gate 32768 and the opt-in decode arm unchanged. |
| [`patches/`](patches/) | **CONSOLIDATED into [`../../archive/work/mmb-general/`](../../archive/work/mmb-general/) on 2026-09-25 — kept as history.**  The fork `gap-closing` commits exported as patches, so the code work survives a fork reset.  On the **r13 rebuild** the campaign is `0001..0014` + `0016..0030`; **`0015` (the shared-NextN MTP fix) was removed 2026-09-24 — it is delivery r13 block 00 and cannot apply on r13**.  `0016` = `QSA_SCORE_WMMA` (default ON), `0017` = MMB quant coverage Q4_0/Q4_1/Q5_0/MXFP4/NVFP4, `0018` = MMB IQ2_S/IQ2_XS/IQ2_XXS (both default ON on non-RDNA4), `0019` = the HC16 F32-elision correctness fix under an eval callback, `0020` = the sparse MTP draft (**OPT-IN**, `LLAMA_MTP_SPARSE=1`), `0021` = the QSA derived-indexer default ON (byte-identical, +9-15 % deep decode), `0022` = the gfx1151 QSA decode crossover 64K → 32K (a 32K-64K decode re-baseline), `0023` = the MMB HC16 per-backend-context activation state + whole-graph consumer scan (fixes nondeterministic MTP at depth; 128K MTP 5/5 one hash == plain), `0024` = the input layer moved to the GPU on a single device (token_embd/mtp_tok_embd GET_ROWS on ROCm0; MTP decode CPU 1090 % -> 155 %, byte-identical; **superseded by `0025`**), `0025` = the host-buffer input layer (restores `integrated = prop.integrated` on HIP + the host-input scheduler guard; `0024`'s `n_devices()==1` heuristic and its ~28 GiB VRAM cost are gone; 8K/40K/128K byte-identical to `0024`), `0026` = the sparse MTP draft prefill **default ON** (`LLAMA_MTP_SPARSE=0` opt-out; pp150K +7.6 %, acceptance unchanged), `0027` = forward the child CUDA `graph_optimize` markings under `-sm tensor` (meta backend: alloc deps before allocation, marks per per-device subgraph after; `MMB_OPT` 0 -> 390/5418), `0028` = the MMVQ↔MMQ band boundary (routed-expert MMVQ band -> 16 + the RDNA4 dense `nrows % 128 != 0` fallback rule; fixes the qwen4exp `n_max` 7->8 cliff: B=9 +33 %, `n_max 8` MTP +8.9 %; kill-switches `GGML_CUDA_DISABLE_MMVQ_MOE_BAND`/`GGML_CUDA_DISABLE_MMVQ_DENSE_BAND`; see [`gfx1151-closing.md`](gfx1151-closing.md)), `0029` = run tiny CPU split graphs (the host-mapped input/PLE `GET_ROWS` + the MTP draft state copies) single-threaded, so the OpenMP active-wait pool is never re-armed and the spec-decode spin is gone: qwen4exp MTP **81.0 → 111.7 t/s**, **~15 → 1.2 cores**, acceptance/same-seed text byte-identical; default-on, kill-switch `GGML_CPU_DISABLE_TINY_GRAPH_SINGLE_THREAD=1`; see [`2026-09-24-mtp-cpu-spin-automatic.md`](2026-09-24-mtp-cpu-spin-automatic.md)), `0030` = opt-in structural input placement (`LLAMA_DEVICE_INPUT=1`: the input layer moves to the output/Meta device, **0 CPU splits**, byte-identical, but the Meta-split GPU gather is **~2.6 % slower** MTP than host + `0029`, so it is not defaulted; see [`2026-09-24-mtp-cpu-spin-structural.md`](2026-09-24-mtp-cpu-spin-structural.md))). |
| [`2026-09-24-gfx1151-mm-band.md`](2026-09-24-gfx1151-mm-band.md) | **The gfx1151 MMVQ band policy, `patches/0031`.**  `0028` splits on gfx1151: the arm-independent B=8→9 step-down is the **dense odd-row MMQ fallback** (widening the gate to RDNA3_5 removes it, +10.0/+9.9/+7.6/+8.7 % at B9/10/11/12), while the **routed MoE band→16 is a loss** (−2..−8 % at B=9..12 across nearly every routed type) so `0031` floors the RDNA3_5 band at 8.  Width probe PASS, `plain == n3`, QSA 26/26, GDN 46/46. |
| [`2026-09-24-gfx1151-mtp-n7-n8.md`](2026-09-24-gfx1151-mtp-n7-n8.md) | **The §4.4 `n7`/`n8`/adaptive matrix on gfx1151, after `0031`.**  `n8` beats `n7` on R/C/K/P (+3.6..+7.4 %); the only `n7` win is X (phase-switching, `n8` acceptance drops).  The `n8` dense-off attribution arm is slower on every axis with acceptance unchanged → the `0031` dense band is the cause.  Adaptive wins R/X, fixed `n8` wins C/K/P; MTP accelerates on every axis. |
| [`2026-09-24-gfx1151-p3-carryover.md`](2026-09-24-gfx1151-p3-carryover.md) | **P3 — the §8 carry-over resolved on gfx1151.**  `0029`/`0030` are no-ops (byte-identical text, CPU 1.36 cores); `0019` imatrix byte-identical default vs `HC16=0`; `0001`/`0008` defaults hold; `0011` `res16` +2.5 %/+2.0 % (`blk16` inert, stays default-OFF); `0005` M-RoPE+image+MTP clean on current **and** pre-`0005` (trigger needs the pre-r13 shared KV → guard retained). |
| [`2026-09-24-gfx1151-input-copy-cost.md`](2026-09-24-gfx1151-input-copy-cost.md) | **§9 step 1 — the input-ring go/no-go.**  Measured the stopgap's per-ubatch host-input copy: **prefill 0.03–0.05 %, decode ~0.5–0.9 % (≈140 µs/token)**, ubatch-independent.  **Verdict: STOP — do not port the ring.**  (`0030` does not change the copies.) |
...
* **Ours:** `archive/work/mmb-general/` — **12 patches**, applied tree
...
* **Current (end of session 12):** `~/llama.cpp` branch **`gap-closing-hostbuf-integrated`** @
  **`df67fd133`** = delivery r13 + the 12 `archive/work/mmb-general/patches/*.patch` + gap-closing
  `0001..0026` (`0023` = the HC16 fix, `0024` = the input-layer stopgap, `0025` = the host-buffer
  input layer that **supersedes `0024`**, `0026` = the sparse MTP draft default ON).  The pre-`0025`
  baseline is branch `gap-closing-r13` @ `73a391aba`.  Details:
  [`2026-09-23-host-buffer-input-layer.md`](2026-09-23-host-buffer-input-layer.md) and
  [`2026-09-23-mtp-sparse-default-on.md`](2026-09-23-mtp-sparse-default-on.md).
* **Current (end of session 10):** `~/llama.cpp` branch **`gap-closing-r13`** @ **`1bb1d794e`**
  (tree `ad7fb9bc…`) = delivery r13 + the 12 `archive/work/mmb-general/patches/*.patch` + gap-closing
...
* `~/llama.cpp` branch **`gap-closing`** @ **`00d8bbbc9`** = `mmb-beta` (r12 `72176ae8a` + the 12
  `archive/work/mmb-general/patches/*.patch`, tree `bca69f23dd…`) + the 2026-09-21/22 changes: **default-on
...
* The campaign's patches are exported to [`patches/`](patches/) in case the local fork branch is
  ...
  64K → 32K)**; `0015` was removed (it is r13 block 00).
...
git am /home/stew675/llama-cpp-rdna-boosts/archive/work/mmb-general/patches/*.patch
git am /home/stew675/llama-cpp-rdna-boosts/archive/work/closing-the-gap/patches/00{01,02,03,04,05,06,07,08,09,10,11,12,13,14}-*.patch   # 0015 was removed; it is r13 block 00
git am /home/stew675/llama-cpp-rdna-boosts/archive/work/closing-the-gap/patches/0016-*.patch   # QSA_SCORE_WMMA, default ON
git am /home/stew675/llama-cpp-rdna-boosts/archive/work/closing-the-gap/patches/0017-*.patch   # MMB Q4_0/Q4_1/Q5_0/MXFP4/NVFP4
git am /home/stew675/llama-cpp-rdna-boosts/archive/work/closing-the-gap/patches/0018-*.patch   # MMB IQ2_S/IQ2_XS/IQ2_XXS
git am /home/stew675/llama-cpp-rdna-boosts/archive/work/closing-the-gap/patches/0019-*.patch   # HC16 F32-elision under an eval callback
git am /home/stew675/llama-cpp-rdna-boosts/archive/work/closing-the-gap/patches/0020-*.patch   # sparse MTP draft (OPT-IN: LLAMA_MTP_SPARSE=1)
git am /home/stew675/llama-cpp-rdna-boosts/archive/work/closing-the-gap/patches/0021-*.patch   # QSA derived-indexer default ON
git am /home/stew675/llama-cpp-rdna-boosts/archive/work/closing-the-gap/patches/0022-*.patch   # gfx1151 QSA decode crossover 64K -> 32K
...
1. **Run the full BETA-TESTING gate suite** — **DONE (session 8)** on gfx1151: Gate 4 MTP
   ...
   [`../../archive/work/mmb-general/BETA-TESTING.md`](../../archive/work/mmb-general/BETA-TESTING.md) §5.
...
too (session 8, `patches/0017`)**: Q4_0/Q4_1/Q5_0/MXFP4/NVFP4, oracles green, PPL parity, pp8192
   ...
   on delivery **r13** (fork `gap-closing-r13`, tip `abf3bff76`), having dropped the superseded WIP
   `0015` (removed 2026-09-24).
...
12. Add `nextn_shared_target_tensors` support — **DONE 2026-09-22 (session 7, the `0015` export, since removed), a
    ...
    WIP `0015` export was removed 2026-09-24 (superseded; un-appliable on r13).
