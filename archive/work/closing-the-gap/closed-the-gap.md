# Closed the gap — completed campaign work (`gap-closing`, sessions 1–10)

**Status:** the completed-work history of the `archive/work/closing-the-gap/` campaign.  The **live handover
and the open items** are in [`closing-the-gap.md`](closing-the-gap.md); read that first.  This file is
the record behind its one-line "done" entries — the dated session narratives, the 2026-09-20 snapshot
body, the appendices and the MTP qualification.
...
Every row is expanded in a section below (newest first).  Patch numbers are the exported campaign
patches in [`patches/`](patches/).

| # | work | patch | headline result |
|---|---|---|---|
| 10 | **Sparse MTP draft** (qwen4exp, opt-in) | `0020` | prefill pp150K **+6.9 %**; memory fixes the plan missed |
| 10 | **QSA derived indexer default ON** | `0021` | **+9.1 % @80K / +14.6 % @150K** deep decode, byte-identical |
| 10 | **gfx1151 QSA decode crossover 64K→32K** | `0022` | 48K sparse +1.8 %, 64K +4.6 % (decode re-baseline) |
| 10 | **MMB HC16 F32-elision root cause** (MTP) | — | the depth-purity blocker is HC16, not the draft |
| 9 | HC16 F32-elision under an eval callback | `0019` | imatrix `in_sum2` byte-identical to `HC16=0` |
| 8 | `QSA_SCORE_WMMA` prefill scorer | `0016` | pp32768 +1.0 %, oracle 225/225 |
| 8 | MMB quant coverage Q4_0/Q4_1/Q5_0/MXFP4/NVFP4 | `0017` | pp8192 +19.5/+22.4/+25.4 % |
| 8 | MMB quant coverage IQ2_S/IQ2_XS/IQ2_XXS | `0018` | dense IQ2_XS pp8192 +16.5 % |
| 7 | QSA prefill scorer trim (`QSA_SCORE_BOUNDS`+`QUERY_STRIP`) | `0014` | pp8192 +0.5 % |
| 7 | shared-NextN MTP fix (upstream #23398) | → r13 block 00 | 0 draft errors, acceptance 0.287 |
| 6 | HC BF16 streams (`blk16`/`res16`, opt-in) | `0011` | +4.9 % pp8192 / +4.8 % pp32768 |
| 6 | `mmb_cvt_f32_bf16` `out_xn` copy | `0012` | +3.3 %/+3.2 %, bit-identical |
| 6 | prefill indexer relu+head-sum fusion | `0013` | pp32768 +1.8 %, bit-identical |
| 5 | `-lzm auto` semantics + managed PLE reader (off) | `0009` | all modes text-identical |
| 5 | MoE BF16 epilogue (`down16`, opt-in) | `0010` | kernel 1479→846 ms |
| 4 | narrow-row RMS norm (`rms_rows_f32`) | `0006` | ~+0.3 %, bit-identical |
| 4 | QSA visibility fold into `umask` | `0007` | +2.4 %/+1.7 % |
| 4 | tall-tile min-M (M=4 HC inject) | `0008` | +0.8 %/+1.1 % |
| 4 | QSA item-3.5 audit (2 of 3 N/A) | — | invariants already held |
| 3 | depthwise conv1d (`gdn-conv`/`ple-conv`) | `0004` | +3.0/+3.2 % qwen4exp, +6.5/+7.1 % 35B-A3B |
| 3 | QSA block-window fix (M-RoPE + MTP) | `0005` | fixes the assert |
| 2 | HC fusions (`hc_combine_norm` matcher + `hc_gate_mix`) | `0003` | +1.5 % + 1.2–1.5 % |
| 2 | ubatch-8192 target decision + `-ub 16384` root cause | — | target protocol set |
| 1 | Phase-1 recon, campaign promoted from `wip/` to `beta/` | — | see the snapshot body |

---

## Session-10 record (2026-09-22): sparse MTP draft, derived-indexer default, decode crossover, HC16 root cause

Full detail: [`2026-09-22-mtp-sparse-draft.md`](2026-09-22-mtp-sparse-draft.md).  This session
implemented the plan in [`PLAN-mtp-sparse-draft.md`](PLAN-mtp-sparse-draft.md) and root-caused the
decode/purity questions that fell out of it.

### Sparse MTP-draft attention (`patches/0020`, OPT-IN `LLAMA_MTP_SPARSE=1`)

The qwen4exp MTP draft attended **dense** over the whole KV cache while the trunk attends sparse QSA;
at ~150K the single dense draft layer was measured to cost 2.1× all twelve sparse trunk layers
(prefill) / 5.6× (decode).  Ported the reference design as **three coupled edits**:

1. `load_arch_hparams` inherits the trunk compress ratio for the nextn layer (the sidecar's nextn is
   layer **48**, `n_layer_all = 49`; the fallback logs `nextn layer 48 compress ratio 0 -> 4`).
2. `llama_model::create_memory` gives the qwen4exp MTP context a `llama_memory_hybrid_idx`
   (`filter_attn = filter_idx = il >= n_layer()`, `filter_recr = false`) instead of a plain KV cache.
3. `graph_mtp` takes its attention input from `build_inp_mem_hybrid()`, builds `top_k` via
   `build_qsa_top_k` and attends via `build_attn_qsa` above the depth gate; the derived-visibility
   mask elision is preserved (`want_derived_vis ? nullptr : mask`).

Plus **two memory fixes the plan missed**, both required to avoid aborts/races:

4. `llama_memory_hybrid_idx` skips the **empty recurrent child** (`hybrid_idx_no_recr`) in
   `prepare`/`seq_rm`/`state_write`/`state_read`/`state_drop` — an empty recurrent `seq_rm` refuses a
   partial rollback, which aborted the server on the first speculative cache trim
   (`failed to remove sequence 0 with p0=5261, p1=-1`).
5. `llama_memory_recurrent::find_slot` stays quiet when no layer bound a state tensor (the empty
   recurrent still needs its `n` bookkeeping, but the verify positions spammed the
   non-consecutive-position warning).

Arms and gates: prefill sparse above `LLAMA_MTP_SPARSE_MIN_KV` (default 32768), decode/verify off
(`LLAMA_MTP_SPARSE_DECODE=1` opts in).  **Prefill pp150K 927.0 → 990.8 t/s (+6.9 %)** — enabling it
from `n_kv > 2051` is a 25 % *loss* (697.3), because the indexer cost is fixed per query while the
dense attention it replaces is `O(n_kv)`.  5K purity `3553e76d3a9e` (plain = off = on), width probe
PASS, MTP acceptance 0.85035 unchanged, oracles green.  **Default off** — but only because of the
HC16 bug below; the draft itself is pure.

### The incremental QSA indexer was OFF by default (`patches/0021`)

The decode-side root cause of "the sparse draft is slower": `llama-memory-hybrid-idx.cpp` set
`derived_enabled = env != nullptr && atoi(env)` — **default OFF** — while the graph side
(`build_qsa_top_k`) has always defaulted its `idx_cache` to ON.  So the derived block-vector pool was
never created and the fused decode score re-pooled/rotated/scored the **whole raw indexer cache**
every step.  Both `AGENTS.md` and the Phase-2 audit describe the cache as default ON — code/docs drift.
Flipping it ON is **byte-identical** and:
...
Kernel profile (40K): the sparse draft attention `flash_attn_qsa` is **0.05 ms/call** vs the dense
`flash_attn_tile` **1.03 ms/call**; the per-step indexer score + top-k was the overhead.  The pool only
engages for F32/BF16/F16 indexer keys, so `-ctk q8_0` keeps the recompute path unchanged.

### gfx1151 QSA decode crossover 64K → 32K (`patches/0022`)

The 64K threshold was sized around the slow recompute sparse decode.  With the pool ON, re-measured
plain decode, interleaved forced-dense vs forced-sparse, `-n 200 --ctx-checkpoints 0`:
...
Threshold moves to **32768** (gfx1201 stays dense-always).  Decode re-baseline for the 32K–64K band
(40K plain `687cec808661` → `8285d12d40ca`), but intra-build `plain == draft-mtp` still holds and the
P=32768 width probe (now exercising sparse decode) passes (`RS=0` and `RS=from_w`).

### The depth-purity blocker is MMB **HC16**, not the draft (diagnosis, no patch)

Both purity symptoms — the 40K sparse-draft divergence and the 128K/150K MTP nondeterminism — are the
campaign's `GGML_CUDA_MMB_HC16` F32-elision (same mechanism as the session-9 eval-callback fix,
`patches/0019`):

| config (qwen4exp IQ4_NL f16) | result |
|---|---|
| 40K, MMB default | plain = draft = `8285d12d40ca`, **sparse = `c0a3bda5dff4`** |
| 40K, `MMB=0` | plain = draft = sparse = **`a40528f24f2d`** |
| 40K, `HC16=0` | plain = draft = sparse = **`8285d12d40ca`** |
| 128K MTP `n_max 1`, default | **5 runs, 5 hashes** |
| 128K MTP `n_max 1`, `MMB=0` | deterministic, `== plain` (`8ab6e5057578`) |
| 128K MTP `n_max 1`, `HC16=0` | deterministic, `== plain` (`770e770ae7d6`) |
| 128K MTP `n_max 1`, `BF16W=0` | still 5-hashes-class racy |

Mechanism: the HC16 marks (`g_mmb_bf16_only`/`_copy`/`_slot`, file-scope in `mmb.cu`) are cleared
only on the next `graph_optimize` (`g_mmb_marks_after_compute` / `g_mmb_marks_first_split`, the latter
keyed on `cgraph->nodes[0]` — an allocator-reused pointer).  The per-compute cache
(`ggml_cuda_mmb_begin_graph`) is cleared every execution, but the marks are not; the MTP loop's
alternating target/draft decodes can skip the optimize that refreshes them, so a producer elides an
F32 that a consumer in another split then reads as never-written/stale.  The **fix is an open item**
(see `closing-the-gap.md`).  Interim workaround: `GGML_CUDA_MMB_HC16=0`.  The delivery r13 (no MMB) is
unaffected.

---

## Dated records — session 7/8 back through the 2026-09-20 snapshot
