...
* Land as **one fork commit** on `gap-closing-r13`, message prefixed `gap-closing WIP: ` (match
  `patches/0016`–`0019`).
* Export with `git -C ~/llama.cpp format-patch -1 --stdout > archive/work/closing-the-gap/patches/0020-<slug>.patch`.
* Write a dated record `archive/work/closing-the-gap/2026-09-2X-mtp-sparse-draft.md` with: symptom/goal, the
  design, the exact gates with results, the perf table, the profile delta, and the traps hit.
* Update `archive/work/closing-the-gap/closing-the-gap.md` START HERE + NEXT SESSION and `README.md` (record
