...
5. `../../archive/work/arch-independent-memory/DERIVED-MASK-DESIGN.md` (**the spec for V3/V4 — the critical path**).
...
|---|---|---|---|
| **W1** | L2 score-chain memory: relu before the 4-D reshape + `n_blocks`-chunked assembly with `ggml_concat` | `archive/work/qwen4exp/qsa-memory/patches/0001-L2a-L2m-qsa-score-memory.patch` (1 file) | compute **6690.40 → 4450.40 MiB/GPU** (ub1024 3346.50 → 2274.35, ub512 1724.56 → 1188.56); bit-identical |
| **W2** | L1 derived QSA block bias + derived visibility, the **mask prune**, and the input-fill null guards (incl. the `attn_k` one) | `archive/work/qwen4exp/qsa-memory/patches/0002-derived-qsa-block-bias.patch` (10 files, +557/−80, base = W1) | compute **4450.40 → 3251.39**, host **1262.70 → 63.69** MiB (ub1024 1675.33/33.64); coherence byte-identical; MTP 0.61616 |
| **W3** | keys-only QSA indexer cache (the indexer V buffer is never read) | `archive/work/qwen4exp/keys-only-indexer/0001-keys-only-qsa-indexer-cache.patch` (3 files, 62 lines) | indexer KV 956.25 → **318.75 MiB**; box 88.58 → **86.70 GiB**; perf parity; broad validation matrix |
| **W4** | ggml-alloc: release view sources whose views are never consumed (the 3b leak) | `archive/work/arch-independent-memory/patches/0001-ggml-alloc-release-unused-view-sources.patch` (+35, `ggml-alloc.c`); PR copy in `upstream/` | repro 56.00 → **16.00 MiB**; no change on current models (latent trap); **upstream-applicable, clean on master `9cf3bf256`** |
...
prefill −1.1 %, decode −0.7 %.  Record: `../../archive/work/arch-independent-memory/V3-DERIVED-KQ-MASK-PLAN.md`
§4.3-§4.5.
...
git apply -R archive/work/block-15-campaign-wins/ab/w4-revert.patch    # or: git apply <the file> (it is a
...
(`archive/work/arch-independent-memory/repro/ggml-alloc-unused-view.c`) must go back to 56.00 MiB.
...
Both are specced in `archive/work/arch-independent-memory/DERIVED-MASK-DESIGN.md` (§2 options, §3 consumers, §5
...
> `../../archive/work/arch-independent-memory/V3-DERIVED-KQ-MASK-PLAN.md` (the spec) +
> `archive/work/arch-independent-memory/patches/0002-DIAGNOSTIC-verify-derived-kq-mask-predicate.patch`
> (the oracle) + `archive/work/arch-independent-memory/logs/`.  Two findings refine the plan below:
...
matrix, validation, the on-by-default follow-up): `../../archive/work/arch-independent-memory/V4-NATIVE-Q8-KV-PLAN.md`.
Patch: `archive/work/arch-independent-memory/patches/0006-v4-native-q8-kv.patch` (6 files, +357/−47, base
W1+W2+diag+2a+2b+2c).
...
### 3.4 The next memory lever: bf16-native MMA K/V — **DONE: shipped as V5** (`../../archive/work/arch-independent-memory/BF16-NATIVE-KV-PLAN.md` §9)
...
1. **Merge** W1–W4 + V3 + V4 into one tree.  W1+W2+V3+V4 are already on the fork tree (22 files,
   ...
   (`archive/work/arch-independent-memory/patches/0001-*`) that also applies to upstream master.  Use
...
5. **Stage it here**: `archive/work/block-15-campaign-wins/block-15-campaign-wins.patch` + the promotion record
...
`archive/work/arch-independent-memory/snapshots/fork-tree-W1-W2-V3-V4-2026-09-10.patch` (3453 lines, applies
...
`archive/work/arch-independent-memory/snapshots/fork-tree-W1-W2-V3-V4-2026-09-10.patch`; per-increment patches
below.

**Campaign patches** (all `git apply --check` clean, in this order over a clean `9113cc188`):
W1 `archive/work/qwen4exp/qsa-memory/patches/0001-L2a-L2m-qsa-score-memory.patch`,
W2 `…/0002-derived-qsa-block-bias.patch` (10 files, +557/−80), W3
`archive/work/qwen4exp/keys-only-indexer/0001-…`, W4 `archive/work/arch-independent-memory/patches/0001-…` + its PR copy
`upstream/UPSTREAM-PR-ggml-alloc-unused-view.patch` (the A/B revert is `ab/w4-revert.patch`), V3
`archive/work/arch-independent-memory/patches/0002-DIAGNOSTIC-…` (the phase-1 oracle, **diagnostic only - never
...
> the reusable driver + KV-capable probe are committed in `../../archive/work/kv-quant-purity-followups/tools/`
...
**Tools** (in-repo): `archive/work/qwen4exp/qsa-memory/tools/{model-sweep.sh,mask-scaling.sh,bufsize.sh,ub-sweep.sh,ab-coherence.sh,mtp-ab.sh,l0a-scheddump.sh,peak-ledger.py,view-reuse-ledger.py}`; repro
`archive/work/arch-independent-memory/repro/ggml-alloc-unused-view.c`.
...
`archive/work/qwen4exp/qsa-memory/tools/` set it; drop it there).  Model paths, the V3 numbers and the V3 launch
recipe are in `archive/work/arch-independent-memory/V3-DERIVED-KQ-MASK-PLAN.md` §4.4.
...
READ FIRST: archive/work/arch-independent-memory/BF16-NATIVE-KV-PLAN.md (the full plan: the measured before-state,
the mechanism with exact call sites, the design, the code map, the validation protocol, the ship rule and
the risks), then archive/work/block-15-campaign-wins/HANDOVER.md section 3.4 (the scope decision D10) and
archive/work/arch-independent-memory/V4-NATIVE-Q8-KV-PLAN.md (V4 is the template this follows - its implementation
...
2. **V4's −1.7 % prefill**: identified as the lost `cp_async` pipeline (a quantized source cannot be
   ...
   `../../archive/work/arch-independent-memory/V4-NATIVE-Q8-KV-PLAN.md` §5 - build it only if the pipeline loss
...
2. re-establish the **dense `--spec-draft-n-max <= 7` (W <= 8) guarantee** for
   ...
   (`archive/work/sm-tensor-plain-vs-spec/logits-dump-singlewidth.cpp`), **never** the
...
| C9 | serving | `--parallel 4` works (the RDNA3_5 V3 fix), gfx1151 pass if the hardware is available (`archive/work/strix-halo/GATE-2026-09-10-block15-rdna35.md` is the template) |
...
Brief + evidence + repro commands: **`../../archive/work/kv-quant-purity-followups/README.md`**; summary in
`README.md` §7.
