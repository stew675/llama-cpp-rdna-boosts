...
> `archive/work/block-15-campaign-wins/BETA-TESTING.md` §4c.  Two instruments worth reusing for any "does the model
...
`archive/work/kv-quant-purity-followups/HANDOVER-2026-09-11-remaining-work.md` §2–§5/§12/§13 (its *plan* is
spent — see the status banner there — but the environment/traps sections are live).  The beta record
is `archive/work/block-15-campaign-wins/README.md` + `BETA-TESTING.md` (§4c is this defect).

## 1. Mission

Block 15 (the attention-memory campaign, staged in `archive/work/block-15-campaign-wins/`) **cannot be
...
BIN=/tmp/blk15x/build-beta/bin tools/qsa-ppl-oracle.sh tensor f16     # tools = archive/work/kv-quant-purity-followups/tools
```
...
* **Not W4.**  `git apply archive/work/block-15-campaign-wins/ab/w4-revert.patch` (35 lines out of
...
1. **Instrument before bisecting.**  Apply `archive/work/kv-quant-purity-followups/tools/node-dump-instrumentation.patch`
...
cp /tmp/b15-new.patch ~/llama-cpp-rdna-boosts/archive/work/block-15-campaign-wins/block-15-campaign-wins.patch
...
behaviour change — the `archive/work/block-15-campaign-wins/` inventory.  Then commit + push **only** to the
...
* Beta: `/tmp/blk15x` @ `d0f71b2e8` (branch `blk15-iq4`), tree `39540b7f4fd8e8569dee64bfa3ee84bf1b20e75d`;
  build `build-beta`; patch `archive/work/block-15-campaign-wins/block-15-campaign-wins.patch` (3 787 lines).
