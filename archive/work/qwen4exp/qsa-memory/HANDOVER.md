...
> ## SUPERSEDED 2026-09-10 — see `../../../archive/work/block-15-campaign-wins/HANDOVER.md`
>
...
> 0015** (`archive/work/block-15-campaign-wins/`, plan + handover).  Upstream-applicable items live in
...
`archive/work/qwen4exp/qsa-memory/tools/` and `archive/work/qwen4exp/qsa-memory/logs/`.
The keys-only patch + its own validation record are in
`archive/work/qwen4exp/keys-only-indexer/`.
...
**Patch:** `archive/work/qwen4exp/keys-only-indexer/0001-keys-only-qsa-indexer-cache.patch` (62 lines, 3 files,
...
- Extended matrix (all clean): bf16 KV @ ctx204800; f32 @ ctx16384 (f32 @204800 is a capacity OOM,
  ...
  f16 A/B byte-identical. Details in `archive/work/qwen4exp/keys-only-indexer/README.md`.
...
5. **Regression set:** the keys-only matrix in `archive/work/qwen4exp/keys-only-indexer/README.md` (bf16/f32,
...
## Appendix B — artifact inventory (`archive/work/qwen4exp/qsa-memory/`)
...
| `archive/work/qwen4exp/keys-only-indexer/` | Deliverable A patch + validation record |
