#!/usr/bin/env python3
"""Target-acceptance-rate controller sweep (GGML_ADAPTIVE_TARGET), vs the credit
bucket (base + tuned `d`).  5-prompt set, 1 GPU, adaptive cap 12."""
...
CANDS = {
 ...
 "t55w32":    dict(T, GGML_MTP_TARGET_PCT="55", GGML_MTP_TARGET_WINDOW="32"),
 "t60w32":    dict(T, GGML_MTP_TARGET_PCT="60", GGML_MTP_TARGET_WINDOW="32"),
 "t65w32":    dict(T, GGML_MTP_TARGET_PCT="65", GGML_MTP_TARGET_WINDOW="32"),
 "t55w64":    dict(T, GGML_MTP_TARGET_PCT="55", GGML_MTP_TARGET_WINDOW="64"),
 "t60w16":    dict(T, GGML_MTP_TARGET_PCT="60", GGML_MTP_TARGET_WINDOW="16"),
}
PROMPTS = [("reasoning", "r1-reasoning.txt"), ("prose", "p1-prose.txt"),
           ("code", "c3-code.txt"), ("code", "c4-code.txt"), ("recall", "k1-recall.txt")]
print(f"{'cand':9s} {'geo':>6s} {'worst':>6s}  " + " ".join(f"{p.split('-')[0]:>6s}" for _, p in PROMPTS), flush=True)
for cn, envx in CANDS.items():
    ...
    print(f"{cn:9s} {geo:6.3f} {min(rs):6.3f}  " + " ".join(cells), flush=True)
print("tune9 done", flush=True)
