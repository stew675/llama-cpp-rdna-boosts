...
2. `archive/work/kv-quant-purity-followups/README.md` — the F1/F2/F3 briefs, the evidence tables, the excluded
...
5. `archive/work/block-15-campaign-wins/README.md` + `HANDOVER.md` §10 (block 15 owns the KV-native staging).
...
* Delivery repo `~/llama-cpp-rdna-boosts`, `main` = **`b5da3cc`** (pushed).  Block 15 beta: tip
  **`0c8099ca2`** (tree `7335b923d`, base `1bcf4e82d`), patch in
  `archive/work/block-15-campaign-wins/block-15-campaign-wins.patch`, `[PATCH 15/15]`.
...
* **Width probe** (the sensitive instrument — the raw-logit hash, *not* text):
  ...
  clang++ -O2 -std=c++17 -I /tmp/canon-llama/include -I /tmp/canon-llama/ggml/include \
    archive/work/kv-quant-purity-followups/tools/logits-dump-kv.cpp -o /tmp/lw-f1 \
  ...
  `archive/work/sm-tensor-plain-vs-spec/p0long.txt`; prints `[L] W=%d logits0_hash=%016llx nv=%d`.
...
* **Node-dump instrument** (found the F2 divergence): `git apply
  archive/work/kv-quant-purity-followups/tools/node-dump-instrumentation.patch`, rebuild `ggml-hip`, run with
...
* Docs: new dated `WORKLOG.md` entry at the top; block notes in `patches/README.md`; update
  ...
  the purity critical-fact bullet), `TODO.md`, `archive/work/kv-quant-purity-followups/README.md`, and the beta
