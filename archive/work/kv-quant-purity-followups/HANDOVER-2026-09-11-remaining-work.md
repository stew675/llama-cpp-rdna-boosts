...
Block 15 is **beta-only**: `archive/work/block-15-campaign-wins/block-15-campaign-wins.patch`, tip
...
| `/tmp/p0long.txt` | only for the issue-25 probe; the *width* probe text lives in the repo (`archive/work/sm-tensor-plain-vs-spec/p0long.txt`, 532 tokens) |
...
All scripts live in `archive/work/kv-quant-purity-followups/tools/` (durable) — copy them to `/tmp` and chmod +x.
...
Block 15 (`archive/work/block-15-campaign-wins/`) is fully revalidated (the 2026-09-11 records in that dir) and
...
* **Issue #25 GDN chunked prefill** (plain vs spec): latent; it does *not* currently break qwen4exp
  (§6.1). Repro/fix directions: `archive/work/issue-25-mtp-batch-width/GDN-CHUNKED-PREFILL-FOLLOWUP.md`; the
...
6. **Re-cut block 15**: apply `archive/work/block-15-campaign-wins/block-15-campaign-wins.patch` to the new tip,
