...
* **Block 15 beta**: base `6f07fe67a` → beta commit `8c377b958`, tree `34527a292` (sixth re-cut,
  ...
  amendment invalidates it and it must be re-cut** (`archive/work/block-15-campaign-wins/`, `HANDOVER.md`
...
1. Apply the existing instrument `archive/work/kv-quant-purity-followups/tools/fa-kernel-chooser-trace.patch`
...
* **The swappable-`.so` A/B works here** (the change is in `ggml-hip`), unlike the recent libllama-side
  ...
  `archive/work/kv-quant-purity-followups/tools/sobench.sh` for an interleaved fixed/base comparison
...
**The follow-on task has its own brief:** restoring the **fused sparse QSA path for the quantized
types** on qwen4exp (the block-14 gate currently forces the dense masked path there, which costs ~13.8 %
