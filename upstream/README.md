...
| `UPSTREAM-PR-moe-router-tie-break.patch` + `.md` | ggml-cuda: make the fused MoE router (`topk_moe`) bit-identical to the generic `soft_max -> argsort -> get_rows -> norm` chain (match the `block_reduce` softmax order, the `reduce_rows_f32` norm order, and `div`), and make the bitonic `argsort` break ties by index (matching the CUB path; the fused router's argmax already does).  The fusion is selected by an address-overlap guard, so before this the model output depended on the allocation plan | **inside block 08** (2026-09-13 seventh amendment, TODO item 19) | prepared 2026-09-13; applies clean to `790cf51aa`; validated on gfx1201 (fused == unfused for all eight native KV types on `-sm tensor` and `-sm layer`, `plain == draft-mtp` holds, `test-backend-ops` 18065/18065, 4B coherence unchanged); no NVIDIA hardware was available; not filed |
